...
Na Vercel, não use `node server.js` como Build Command nem como função. A Vercel executa funções serverless dentro de `api/`; por isso o projeto contém `api/profile-picture.js` e `vercel.json`. O `server.js` continua disponível apenas para execução local. O `vercel.json` define `start/` como `outputDirectory`, pois esse é o diretório dos arquivos estáticos.
